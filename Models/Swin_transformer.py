...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)
        #self.act_loss = self.hoyer_loss((x/self.softmax.up)*t, t)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # FFN
        x = x + self.drop_path(self.mlp(self.norm2(self.act(x))))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.BatchNorm2d, downsample=None, use_checkpoint=False,
                 fused_window_process=False):
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for layer in self.layers:
            ...
            #print(layer.blocks)
            #if isinstance(layer, MyFloor):
            #    self.neuron_count += torch.numel(x)
            #    self.act_loss += self.hoyer_loss((x/layer.up)*t, t)
